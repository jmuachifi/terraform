...
resource "google_service_account" "gke_nodes" {
  account_id   = "gke-staging-nodes"
  display_name = "GKE Staging Node Pool"
}
...
locals {
  computed_node_pools = length(var.node_pools) > 0 ? [
    for pool in var.node_pools : merge(pool, {
      ...
      labels          = merge({ env = "staging" }, try(pool.labels, {}))
    })
    ] : [
    {
...
output "gke_cluster_name" {
  ...
  description = "Cluster name for staging environment."
}
